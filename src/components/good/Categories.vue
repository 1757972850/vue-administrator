<template>
  <div id="category">
    ...
    <el-card>
      <!-- 引用表格组件【需要理解】 -->
      <zk-table :data="cateList" :columns="columns" :selection-type="false" :expand-type="false" :style="{ fontSize: '14px', color: '#777' }" show-index index-text="#" border>
      ...
      <el-dialog title="添加分类" :visible.sync="addVisible" width="40%" @close="addClosed()">
        <el-form :model="addCateForm" :rules="Rules" ref="addCateRef" label-width="100px">
          ...
          <el-form-item label="父级分类">
            <el-cascader :options="parentList" :props="cascaderProps" v-model="selectedKeys" clearable placeholder="请选择父级分类" @change="parentChanged()"> </el-cascader>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        {
          //表格每列的数据: label列标题/props列内容对应的属性名/type列类型/template模板对应的作用域插槽名
          label: '分类名称',
          prop: 'cat_name',
        },
        {
      ...
      addCateForm: {
        //添加的分类请求的参数: 分类名称/父级分类的id/分类等级 (不能为空)
        cat_name: '',
        cat_pid: 0,
        cat_level: 0,
      },
      ...
      cascaderProps: {
        //级联选择器的配置对象: value真正选中的属性/label显示的属性/children嵌套的属性/悬停触发/可单选
        value: 'cat_id',
        label: 'cat_name',
        children: 'children',
        expandTrigger: 'hover',
