<template>
  <div id="params">
    ...
    <el-card>
      ...
      <div>
        <span>请选择商品分类 : </span>
        <el-cascader :options="cateList" :props="cascaderProps" v-model="selectedKeys" clearable placeholder="请选择商品分类" @change="cateChanged()"> </el-cascader>
      </div>
      <!-- Tabs标签页 -->
          ...
          <el-table :data="manyParams" border stripe>
            ...
            <el-table-column type="expand" width="50px">
              <template slot-scope="scope">
                <!-- tag标签显示 -->
                <el-tag type="success" v-for="(item, index) in scope.row.attr_vals" :key="index" closable @close="deleteTag(index, scope.row)" class="expend">
                  {{ item }}
                </el-tag>
                <!-- 添加tag标签与显示文本框的切换 -->
                <el-input v-if="scope.row.inputVisible" v-model="scope.row.inputValue" ref="saveTagInput" class="change-input" size="small" clearable @blur="handleInputConfirm(scope.row)"> </el-input>
...
export default {
  ...
  data() {
    return {
      ...
      cascaderProps: {
        //级联选择器的配置对象: value真正选中的属性/label显示的属性/嵌套的属性
        value: 'cat_id',
        label: 'cat_name',
        children: 'children',
        expandTrigger: 'hover',
  ...
  methods: {
    ...
    handleInputConfirm(sR) {
      if (sR.inputValue.trim().length) {
        //有效数据则Push到attr_vals数组
