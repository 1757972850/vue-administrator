  ...
  <div id="roles">
    ...
    <el-card>
      <el-table :data="roleList" border stripe>
        ...
        <el-table-column type="expand" width="50px">
          <template slot-scope="scope">
            <div class="areas">
              <!-- 渲染权限: 栅格布局区域 -->
              <el-row v-for="(item1, i1) in scope.row.children" :key="item1.id" :class="['bdBottom', i1 === 0 ? 'boTop' : '', 'vcenter']">
                <!-- 渲染一级权限 -->
                <el-col :span="5">
                ...
                <!-- 渲染二级与三级权限 -->
                <el-col :span="19">
                  <el-row v-for="(item2, i2) in item1.children" :key="item2.id" :class="[i2 === 0 ? '' : 'boTop', 'vcenter']">
                    <!-- 二级权限 -->
                    <el-col :span="5">
                    ...
                    <!-- 三级权限 -->
                    <el-col :span="14">
                      <el-tag type="warning" closable v-for="item3 in item2.children" :key="item3.id" @close="deleteRightWarn(scope.row.id, item3.id)">
                        {{ item3.authName }}
                      </el-tag>
                    </el-col>
                  </el-row>
      ...
      <el-dialog title="分配权限" :visible.sync="assignVisible" width="40%" @close="assignClosed()">
        <!-- 树形控件 -->
        <el-tree :data="rightForm" :props="treeProps" show-checkbox node-key="id" default-expand-all :default-checked-keys="defaultKeys" ref="treeRef"> </el-tree>
        <span slot="footer">
